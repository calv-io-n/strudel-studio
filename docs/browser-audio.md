...
Choose **Record to** in the composition toolbar, then press the top **Record** button. It connects the selected microphone/interface if necessary and starts recording alongside the composition at the playhead. If the composition is already playing, recording joins it. Press **Record** again or either transport Stop control to finish. No sample-library form or separate Save action is required.
...
**Recording settings** contains dry/editable capture and latency compensation; use the top metronome for count-in. Defaults are processed capture, no count-in, and zero compensation. Device/channel selection stays in Audio input; it is not duplicated in another recorder form. Highlighted-instrument recording remains a separate feature.
