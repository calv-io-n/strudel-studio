...
- **Pace** sets how fast the phrase plays against its natural speed: **½×** (half time), **¾×**, **Natural**, **1½×**, **2×**. Each stretches the phrase uniformly, then snaps its length to whole bars (or beats, from the *snap to* select) so it occupies a musical length, and sets the clip length to match; the summary line shows the result, for example "6 bars · 1.05× natural pace · pitch preserved". Type a bar count and press **Fit** for an explicit length. A preset that cannot be honoured within the 0.5×–2× limit explains the allowed range instead.
- **Start at first attack** moves the phrase start to its first detected attack while keeping the beat it starts on, so an entry lands squarely on the bar.
- On the timeline, right-click an audio clip for **Pace: natural / half time / double time** without opening the dialog.
