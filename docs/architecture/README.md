...
Quick start is a native dialog containing a five-concept carousel, mounted by `studio/client/quick-start.ts`. Previous/Next buttons, arrow keys, and topic links select one visible slide; input and composition help buttons open the corresponding slide directly. Hidden-slide video pauses, reduced-motion preferences disable automatic animation, and closing returns focus to the invoking control. Instructions and media are static assets with no project-schema or storage changes. `scripts/record-drag-guide.ts` records the real tab-to-track gesture in an isolated Playwright context and verifies the created clip survives save/reload. See [art sources and regeneration](../design/quick-start-art.md).

Project schema v8 migrates v1–v7; v6 added live input, applied code snapshots and precision metadata, v7 pattern tempo overrides, and v8 replaces the seconds-based take timing fields with clip anchors (ADR 0010). IndexedDB v2 adds pending work without clearing existing stores. Save revisions reject stale writes. Existing ZIP backups preserve project/sample identities; restoration validates all collisions before a single transaction publishes audio and a newly named session. Cross-device and cross-origin migration is explicit backup/restore. Browser storage can be cleared or reach quota; the UI reports unsaved state and offers backups and persistent-storage requests.
