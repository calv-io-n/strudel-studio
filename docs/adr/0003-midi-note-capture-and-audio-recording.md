...
Transcribe also offers Jam with composition: repeat a selected range while excluding all composition clips sourced from the destination tab, leaving its sound available for live MIDI performance. Infer Strudel syntax only from that live input. This is a temporary audition mix, restored on exit without modifying stored mutes or Solo; it extends the normal finite composition transport only for this explicit mode.
