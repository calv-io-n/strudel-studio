...
- In Transcribe, offer **Jam with composition**: loop a chosen composition range (defaulting to the full arrangement) while temporarily excluding every clip sourced from the current destination tab. Keep the other tabs playing as accompaniment and route live MIDI through the destination sound and effects. Transcribe only the incoming performance into Strudel syntax, with a code preview that updates while playing; accompaniment is not transcribed. This action explicitly starts the loop when stopped. Show the loop range and the temporarily excluded tab, keep the destination pinned, and restore its normal playback eligibility on leaving the mode, cancel, or Stop without altering saved track/clip mutes or Solo. Respect the existing mix for the remaining tabs.
