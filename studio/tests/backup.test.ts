...
test('backup restores referenced audio and creates a collision-safe session', async () => {
  ...
  try {
    ...
    const restored = await restoreBackup(bytes, target); assert.equal(restored.project.version, 8); assert.deepEqual(restored.missing, []); assert.equal(restored.project.midiSound, project.midiSound); assert.deepEqual(restored.project.midiInstrument, project.midiInstrument);
...
test('v3 sessions migrate and sample insertion preserves surrounding code without duplicating effects', () => {
  ...
  const next = ProjectSchema.parse({ ...old, version: 3 }); assert.equal(next.version, 8); assert.deepEqual(next.assetIds, []);
