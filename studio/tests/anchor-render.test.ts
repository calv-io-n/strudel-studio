...
test('rendering stretches between anchors, keeps pitch and stereo, and ends at the last anchor',async()=>{
 const rendered=decodeEditableWav(new Uint8Array(await renderAnchoredAudio(audio(tone(3)),anchors,bpm)));
 assert.equal(rendered.left.length,48000);assert.deepEqual(rendered.left,rendered.right);
 for(const [start,end] of [[.2,1],[1.4,1.9]])assert.ok(Math.abs(crossings(rendered.left,start,end)-220)<5,`${start}-${end}`);
 let jump=0;for(let i=rate*1.2;i<rate*1.9;i++)jump=Math.max(jump,Math.abs(rendered.left[i]-rendered.left[i-1]));assert.ok(jump<.15);
});
...
test('rendering starts at the first anchor, unstretched intervals are sample-exact, and no tail is copied',async()=>{
 ...
 assert.equal(rendered.left.length,24000);
 for(let k=rate*.1;k<rate*.9;k+=97)assert.ok(Math.abs(rendered.left[k]-signal[12000+k])<1e-6,`frame ${k}`);
});
