...
export class RecordingPanel {
  ...
  constructor(private engine: Engine, private prepareInternal: () => Promise<void>, private saved: (asset: Asset) => Promise<void>, private report: (message: string) => void, private session: () => string, private stopInternal: () => void) {
  ...
  private async setup() {
    ...
    this.capture = new AudioTakeCapture(this.context!, this.input!, peak => { this.el<HTMLMeterElement>('level').value = peak; this.el('level').title = peak >= .99 ? 'Clipping — lower input gain' : 'Input level'; }, chunk => { if (this.recoveryPrefix) void writePending(`${this.recoveryPrefix}chunk:${String(this.chunkIndex++).padStart(8, '0')}`, chunk).catch(() => this.status('Recording · browser recovery unavailable; stop and save before closing.')); });
  ...
  private async start() {
    ...
    await writePending(`${this.recoveryPrefix}meta`, { source: this.source, bpm: this.bpm, offset: this.offset, rate: this.context!.sampleRate, tabId: this.pinnedTab });
  ...
  async restore() {
    const prefix = `audio:${this.session()}:`; const meta = await readPending<{ source: 'internal' | 'external'; bpm: number; offset: number; rate: number; tabId: string }>(prefix + 'meta');
    if (!meta) return; const chunks = await readPendingPrefix<{ left: Float32Array; right: Float32Array }>(prefix + 'chunk:'); if (!chunks.length) return;
    ...
    this.context = new AudioContext({ sampleRate: meta.rate }); this.capture = new AudioTakeCapture(this.context, this.context.createGain(), () => {}); this.capture.restore(chunks);
    this.root.hidden = false; this.el<HTMLSelectElement>('source').value = meta.source; this.el<HTMLInputElement>('trim-end').value = this.capture.duration.toFixed(3); this.preview(); this.status('Recovered audio take · review incomplete capture before saving');
  }
