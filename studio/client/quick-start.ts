...
export function installQuickStart() {
  ...
  dialog.innerHTML = `
    ...
    <div class="quick-start-sections">
      <section id="help-inputs" tabindex="-1"><span class="quick-start-number" aria-hidden="true">01</span><h3>Connect your MIDI keyboard.</h3><p><strong>MIDI keyboard:</strong> choose <b>Connect MIDI</b> beside recording or in the <b>MIDI</b> editor. Allow browser access; a single keyboard connects immediately. With several inputs, check the devices you want to use. The same controls are available in <b>MIDI &amp; on-screen controller</b> in Search. Play a key to see activity right where you connected.</p><p>Open the <b>MIDI</b> tab at the bottom of the editor to choose its sound and effects. Use <b>Apply instrument</b> after editing the code.</p><p class="quick-start-note">Your sessions are stored in this browser. Use the + beside the session name to start your own; download a project backup to keep a separate copy.</p></section>
      ...
      <section id="help-audio" tabindex="-1"><span class="quick-start-number" aria-hidden="true">03</span><h3>Record your microphone.</h3><ol><li>Open the <b>Input</b> tab, choose <b>Connect microphone</b> and allow browser access. Pick your device and channel under <b>Settings</b>, then check the level meter. Turn on <b>Monitor input</b> if you want to hear it through Studio.</li><li>Choose <b>● Record</b> in the top bar, enable <b>Audio input</b>. Enable <b>MIDI</b> too to record both together. Choose <b>Tab</b> for the open pattern, or <b>Composition</b> and select its track and pattern placement.</li><li>Choose the top <b>Record</b> button, perform, then stop the recording. The take appears as faint code awaiting approval. Choose <b>Preview take</b> to listen, then <b>Keep take</b> to save or <b>Discard</b> to leave the pattern unchanged. Keeping updates every composition placement of that pattern.</li></ol><p class="quick-start-note">Monitoring is listening, not recording. Stop turns monitoring off; turn it on again when needed. A muted track or another soloed track can also silence monitoring.</p></section>
      <section id="help-midi" tabindex="-1"><span class="quick-start-number" aria-hidden="true">04</span><h3>Test a sound. Record a phrase.</h3><ol><li>Click the outlined <code>note()</code> name. <b>Test MIDI</b> auditions that instrument without saving a take.</li><li>Choose <b>Record MIDI on pattern</b> to hear the entire pattern, or <b>Record MIDI solo</b> to hear only your performance. Both arm the clicked phrase in the top Record bar without disabling Audio input. You can also enable MIDI directly in the Record bar to append new notes; no selected note is required.</li><li>Press the top <b>Record</b> button when ready. The enabled metronome counts four beats first. Code takes shape in the editor as notes arrive. Pulsing placeholders mark audio still being captured. Finished code stays faint until you keep the take.</li><li>Press <b>Stop</b>, preview, and <b>Keep take</b>. The source pattern changes in every placement; unrelated code stays intact.</li></ol><p class="quick-start-note">MIDI and Audio input are independent toggles. With both enabled, MIDI updates the selected note (or appends a new section) and audio appends below it in the same pattern. Switching tabs does not redirect the take. Use Recording destination or Audio section to return; your scrolling is respected. Discard keeps your original phrase. The demonstrations use virtual MIDI, not a physical-device test.</p></section>
      ...
      <section id="help-tempo" tabindex="-1"><span class="quick-start-number" aria-hidden="true">06</span><h3>One project clock.</h3><p>Change <b>BPM</b> beside the transport while stopped. Every pattern has a read-only tempo header; the rest of your code stays editable. One Studio cycle spans four beats. Click the metronome beside <b>Record</b> to cycle through <b>Off</b> (gray), <b>Count-in only</b> (yellow), and <b>Continuous</b> (yellow with a loop badge), then back to Off. Both enabled modes count four beats before playback or recording. Continuous keeps clicking along at project BPM; Stop silences it and keeps your mode preference.</p><p>Patterns inherit project tempo. For an optional override, open the pattern’s context menu and choose <b>Pattern tempo</b>. It affects every placement; choose <b>project</b> to restore inheritance. Recorded audio keeps its natural speed and pitch.</p><p class="quick-start-note">Double-click a pattern tab or press F2 to rename. Right-click or press Shift+F10 for its other actions. Search starts with pattern tabs, Sample Catalogue, MIDI and Audio input settings, Full song render, Quick Start, Strudel-file import, and Import GitHub Samples.</p></section>
    </div>`;
  const illustrations = [
    { topic: 'inputs', file: 'connect-midi-3d', alt: 'A faceless 3D character connects a MIDI keyboard to a laptop by USB.', caption: 'Connect the keyboard. Choose Connect MIDI. Check for incoming notes.', demo: '<div class="guide-signal"><span>MIDI keyboard</span><span class="guide-cable"><i></i></span><span>Laptop</span></div><span class="guide-connected">● MIDI input received</span>' },
    { topic: 'mapping', file: 'connect-midi-3d', alt: 'A MIDI keyboard connected to a laptop.', caption: 'Connect a controller, then bind its knob from slider().', demo: '<div class="guide-signal"><span>Controller knob</span><span>→</span><span>slider()</span></div>' },
    ...
    { topic: 'midi', file: 'transcribe-midi-3d', alt: 'A faceless 3D character plays a MIDI keyboard as note blocks become code on a monitor.', caption: 'Record MIDI → Stop → Keep take. Your performance becomes code.', demo: '<div class="guide-notes"><span>C4</span><span>E4</span><span>G4</span><b>→</b></div><code class="guide-code">note("c4 e4 g4").s("triangle")</code>' },
  ];
  ...
  browserDemo.innerHTML = `<video controls muted loop playsinline preload="none" poster="${import.meta.env.BASE_URL}help/drag-pattern-browser.png" aria-label="Actual Strudel browser demonstration: cursor grabs Pattern 1, drags it down, and drops it onto Track 1"><source src="${import.meta.env.BASE_URL}help/drag-pattern-browser.webm" type="video/webm">Your browser cannot play this recording. Follow the written drag-and-drop instructions below.</video><figcaption>Grab Pattern 1, drag down to Track 1, and release to create a clip. The crop keeps the tab, destination, and saved clip in view.</figcaption>`;
  dialog.querySelector('#help-arrange h3')!.after(browserDemo);
  const clips = [
    { topic: 'inputs', file: 'connect-midi-browser', caption: 'Connect MIDI beside recording. One keyboard connects after permission; choose inputs directly when several are available. Virtual controller demonstration.' },
    { topic: 'midi', file: 'test-midi-browser', caption: 'Test MIDI: audition the selected sound without a take.' },
    { topic: 'audio', file: 'record-both-browser', caption: 'Audio input + MIDI: enable both, Record once, Stop once, Keep take. Both sections stay in one pattern. This demonstration uses virtual inputs.' },
    { topic: 'midi', file: 'record-pattern-browser', caption: 'Record MIDI on pattern: arm the phrase, press Record, then Keep take.' },
    { topic: 'midi', file: 'record-solo-browser', caption: 'Record MIDI solo: the same destination, without pattern accompaniment.' },
    { topic: 'tempo', file: 'metronome-modes-browser', caption: 'Click to cycle: gray Off → yellow Count-in only → yellow Continuous with a loop badge → Off.' },
    { topic: 'mapping', file: 'bind-slider-browser', caption: 'Bind MIDI control from slider(), then move the mapped knob. Virtual MIDI demonstration.' },
  ];
  for (const clip of clips) {
    const figure = document.createElement('figure'); figure.className = 'quick-start-visual quick-start-browser';
    figure.innerHTML = `<video controls muted playsinline preload="none" poster="${import.meta.env.BASE_URL}help/${clip.file}.png" aria-label="${clip.caption}"><source src="${import.meta.env.BASE_URL}help/${clip.file}.webm" type="video/webm">Follow the written instructions below.</video><figcaption>${clip.caption}</figcaption>`;
    dialog.querySelector(`#help-${clip.topic}`)!.append(figure);
  }
  ...
  slides.forEach((slide, index) => {
    ...
    const media = document.createElement('div'); media.className = 'quick-start-media';
    for (const child of [...slide.children]) { if (child.matches('figure')) media.append(child); else copy.append(child); }
    if (media.childElementCount) slide.append(media); slide.append(copy);
  ...
  const setPaused = (paused: boolean) => {
    ...
    for (const video of videos) if (paused || !dialog.open || video.closest<HTMLElement>('section')!.hidden) video.pause();
  };
  ...
  dialog.addEventListener('close', () => {
    videos.forEach(video => video.pause()); returnFocus?.focus();
