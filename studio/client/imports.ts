...
export class SampleImports {
  ...
  private packId = crypto.randomUUID();
  constructor(private saved: (asset: Asset) => Promise<void>, private report: (message: string) => void) {
  ...
  private render() {
    ...
    for (const entry of this.entries) {
      ...
      row.className = 'import-row'; check.type = 'checkbox'; check.checked = entry.selected; check.disabled = !!entry.asset || !!entry.error; check.onchange = () => { entry.selected = check.checked; };
  ...
  private async commit() {
    ...
    try {
      for (const [index, entry] of this.entries.entries()) {
        ...
        try {
          ...
          const metadata = { name: entry.path, label: entry.path.split('/').pop()!.replace(/\.[^.]+$/, '').slice(0, 80) || 'Sample', originalBytes: original.byteLength, originalFormat: entry.path.split('.').pop()!.toLowerCase(), pack: packName ? { id: this.packId, name: packName, folder: entry.path.split('/').slice(0, -1).join('/') } : undefined, source: entry.source, provider: entry.provider || 'upload' };
          ...
          entry.asset = result.asset; entry.status = result.reused ? 'Already imported · reused existing sound' : 'Imported'; await this.saved(result.asset);
    ...
    } finally { await context.close(); this.running = false; }
  }
}
