...
Open **WAV editor** from the import page or command palette to extract samples from a local WAV. Existing library sounds offer **Options → Edit sample**. Drag a waveform region or enter start/end times, preview it, then **Save sample**, **Save and insert**, or **Download WAV**. Each save creates an independent sample (identical audio is reused); sources and existing pattern references stay unchanged. Audio is processed on your device. The source stays available while navigating Studio, but unsaved selections are lost on reload. Source limits: mono/stereo PCM or float WAV, 256 MB, 15 minutes, and 256 MB decoded audio. Extracted float32 WAV samples are limited to 64 MB. Use a shorter selection if needed.
