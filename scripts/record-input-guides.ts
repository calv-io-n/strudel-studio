...
async function setup(connect = true) {
  ...
  if (connect) { await page.locator('#palette-open').click(); await page.locator('#command-palette input').fill('MIDI & on-screen controller'); await page.keyboard.press('Enter'); await page.locator('#midi-settings-connection [data-midi-enable]').click(); await page.waitForFunction(() => document.querySelector('#midi-settings-connection [data-midi-status]')?.textContent?.startsWith('MIDI ·')); await page.keyboard.press('Escape'); }
...
async function record(name: string, mode: 'test' | 'pattern' | 'solo' | 'slider' | 'metronome' | 'both' | 'connect') {
  const { context, page } = await setup(mode !== 'connect');
  const frames: Buffer[] = [];
  ...
  if (mode === 'connect') {
    await click(page.locator('#record-toggle')); await click(page.locator('[data-capture=midi]'));
    await page.screenshot({ path: `${output}/${name}.png`, clip: crop });
    await click(page.locator('#record-midi-connection [data-midi-enable]'));
    await page.evaluate(() => (window as any).guideMidi(144, 60, 100)); await hold(600); await page.evaluate(() => (window as any).guideMidi(128, 60, 0)); await hold(600);
    ...
    if (mode !== 'test') {
      ...
      await expect(page.locator('.pending-code').first()).toHaveAttribute('aria-label', /Recording/);
    }
    ...
    if (mode !== 'test') { if (mode === 'both') { await expect(page.locator('#record-retry')).toHaveText('Keep take'); await click(page.locator('#record-retry')); await expect(page.locator('#record-status')).toContainText('Audio saved in pattern'); } else { await expect(page.locator('.pending-code')).toHaveAttribute('aria-label', 'Ready to review'); await click(page.locator('.performance-panel [data-accept]')); } await expect(page.locator('.performance-panel')).toBeHidden(); await page.screenshot({ path: `${scratch}/${name}-saved.png` }); await expect(page.locator('.tab-editor:not([hidden]) .cm-content')).toContainText('note(60)'); }
    await hold(900);
  }
...
try {
  for (const [name, mode] of [['connect-midi-browser', 'connect'], ['record-both-browser', 'both'], ['test-midi-browser', 'test'], ['record-pattern-browser', 'pattern'], ['record-solo-browser', 'solo'], ['bind-slider-browser', 'slider'], ['metronome-modes-browser', 'metronome']] as const) { if (!process.env.STUDIO_GUIDE_ONLY || name === process.env.STUDIO_GUIDE_ONLY) await record(name, mode); }
