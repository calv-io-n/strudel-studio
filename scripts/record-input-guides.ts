...
async function setup() {
  ...
  await page.locator('#add-session').click(); await page.locator('#edit-name').fill('MIDI practice'); await page.locator('#edit-dialog button[value=confirm]').click();
...
async function record(name: string, mode: 'test' | 'pattern' | 'solo' | 'slider' | 'metronome') {
  ...
  if (mode === 'metronome') {
    ...
    if (mode !== 'test') {
      ...
      await expect(page.locator('.pending-code')).toContainText('Recording');
    }
    ...
    if (mode !== 'test') { await expect(page.locator('.pending-code')).toContainText('Ready to review'); await click(page.locator('.performance-panel [data-accept]')); await expect(page.locator('.performance-panel')).toBeHidden(); await expect(page.locator('.tab-editor:not([hidden]) .cm-content')).toContainText('note(60)'); }
    await hold(900);
  }
...
try {
  for (const [name, mode] of [['test-midi-browser', 'test'], ['record-pattern-browser', 'pattern'], ['record-solo-browser', 'solo'], ['bind-slider-browser', 'slider'], ['metronome-modes-browser', 'metronome']] as const) { if (!process.env.STUDIO_GUIDE_ONLY || name === process.env.STUDIO_GUIDE_ONLY) await record(name, mode); }
